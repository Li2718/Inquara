...
describe("new canvas draft cache", () => {
  ...
  it("keeps the starter message until the created canvas confirms it was sent", () => {
    savePendingStarterMessage("canvas-1", "Start from this question");

    expect(getPendingStarterMessage("canvas-1")).toBe("Start from this question");
    expect(getPendingStarterMessage("canvas-1")).toBe("Start from this question");

    clearPendingStarterMessage("canvas-1");

    expect(getPendingStarterMessage("canvas-1")).toBeNull();
  });

  it("clears the pre-create draft without clearing pending starter messages", () => {
    saveNewCanvasDraft("Draft");
    savePendingStarterMessage("canvas-1", "Question");

    clearNewCanvasDraft();

    expect(getNewCanvasDraft()).toBe("");
    expect(getPendingStarterMessage("canvas-1")).toBe("Question");
  });
});
